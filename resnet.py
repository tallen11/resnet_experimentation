...
class ResNet:
    def __init__(self, n):
        ...
        l_norm1 = tf.contrib.layers.batch_norm(self.inputs)

        W_conv = tf.Variable(tf.truncated_normal([3,3,3,16], stddev=0.1))
        b_conv = tf.Variable(tf.constant(0.1, shape=[16]))
        l_conv = tf.nn.conv2d(l_norm1, W_conv, strides=[1,1,1,1], padding="SAME")
        l_conv = tf.nn.elu(l_conv + b_conv)
        ...
        l_downsample1 = tf.nn.elu(l_downsample1 + b_downsample1)
        ...
        # l_downsample2 = tf.nn.elu(l_downsample2 + b_downsample2)
        ...
        l_norm_p = tf.contrib.layers.batch_norm(l_downsample1)
        l_poolf = tf.nn.avg_pool(l_norm_p, ksize=[1,2,2,1], strides=[1,2,2,1], padding="SAME")
        ...
        l_fc1 = tf.nn.elu(tf.nn.xw_plus_b(l_pool_flat, W_fc1, b_fc1))

        W_fc2 = tf.Variable(tf.truncated_normal([512,10], stddev=0.1))
        b_fc2 = tf.Variable(tf.constant(0.1, shape=[10]))
        l_fc2 = tf.nn.elu(tf.nn.xw_plus_b(l_fc1, W_fc2, b_fc2))
    ...
    def __residual_unit(self, unit_input, filter_shape):
        l_norm1 = tf.contrib.layers.batch_norm(unit_input)
        W_conv1 = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1))
        b_conv1 = tf.Variable(tf.constant(0.1, shape=[filter_shape[3]]))
        l_conv1 = tf.nn.conv2d(l_norm1, W_conv1, strides=[1,1,1,1], padding="SAME")
        l_activ1 = tf.nn.elu(l_conv1 + b_conv1)

        l_norm2 = tf.contrib.layers.batch_norm(l_activ1)
        ...
        l_activ2 = tf.nn.elu(l_conv2 + b_conv2)
